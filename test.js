...
var init = exports.init = function(callback) { // This function is used to load appium drivers
    try {
        if(gulpfile.device_list()==1) {
            var serverConfig = config.servers.local;
        }
        if(gulpfile.device_list()==2)
        {
            var serverConfig = config.servers1.local;
        }
        if(gulpfile.device_list()==3)
        {
            var serverConfig = config.servers1.local;
        }
        if(gulpfile.device_list()==4)
        {
            var serverConfig = config.servers1.local;
        }
        ...
        var desired = _.clone(config.environments.android18);
        desired.app = config.build_file_path;
...
var chrome_desktop = exports.chrome_desktop = function(callback) {
    try {
        GLOBAL.service = new chrome.ServiceBuilder(path).build();
        chrome.setDefaultService(service);

        GLOBAL.desktopdriver = new webdriver.Builder()
            .withCapabilities(webdriver.Capabilities.chrome())
            .build();

        desktopdriver.get("http://www.google.com");
        setTimeout(function() {
            callback();
        }, 20000);
            ...
            try {
                item.call().done(function(output) { //call from cases file to perform
                    if (output == "Pass") { // if the child item return value as "Pass" then will it execute another child item else it will execute the another flow and make the test case as fail
                        print = "yes";
                        stackify.next(index, "Function got Executed: " + index); // This will execute another child item
                    } else {
                        print = "no";
                        Fail++
                        ++i
...
var run_desktop_chrome = exports.run_desktop_chrome = function(callback) {
